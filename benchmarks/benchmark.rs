...
pub fn full_benchmark(k: usize) {
    let (duration_sequential, accurate_predictions, accuracy, correct_counts, total_counts) =
        run_sequential_benchmark(k);
...
fn benchmark(k: usize, predict: PredictFn) -> BenchmarkResults {
    ...
    let (accurate_predictions, accuracy) =
        shared::calculate_accuracy(&predicted_labels, &test_images);
    let (correct_counts, total_counts) =
        calculate_accuracy_per_label(&test_images, &predicted_labels);
...
fn calculate_accuracy_per_label(
    test_images: &Vec<Image>,
    predicted_labels: &Vec<u8>,
) -> ([u32; 10], [u32; 10]) {
